...
class PointNet2FeatureExtractor(nn.Module):
    def __init__(self):
        ...
        )  # 512 from sa4, 256 from sa3
        self.fp3 = PointNetFeaturePropagation(
            in_channel=256 + 128, mlp=[256, 256]
        )  # 256 from fp4, 128 from sa2
        self.fp2 = PointNetFeaturePropagation(
            in_channel=256 + 64, mlp=[256, 128]
        )  # 256 from fp3, 64 from sa1
        self.fp1 = PointNetFeaturePropagation(
            in_channel=128 + 3, mlp=[128, 128]
        )  # 128 from fp2, 3 from l0_points (initial xyz)
...
class GRNet_2_2D(torch.nn.Module):
    def __init__(self, cfg=None):
        super(GRNet_2_2D, self).__init__()
        self.gridding_scales = (128, 128, 1)
        ...
        self.conv1 = torch.nn.Sequential(
            torch.nn.Conv3d(
                1, 32, kernel_size=(3, 3, 1), padding=(1, 1, 0)
        ...
        self.conv2 = torch.nn.Sequential(
            torch.nn.Conv3d(32, 64, kernel_size=(3, 3, 1), padding=(1, 1, 0)),
        ...
        self.conv3 = torch.nn.Sequential(
            torch.nn.Conv3d(64, 128, kernel_size=(3, 3, 1), padding=(1, 1, 0)),
        ...
        self.conv4 = torch.nn.Sequential(
            torch.nn.Conv3d(128, 256, kernel_size=(3, 3, 1), padding=(1, 1, 0)),
        ...
        self.fc5_in_features = (
            256 * (self.gridding_scales[0] // 16) * (self.gridding_scales[1] // 16) * 1
        )  # Z_dim is 1
        ...
        self.fc6_reshape_shape = (
            -1,
            256,
            self.gridding_scales[0] // 16,
            self.gridding_scales[1] // 16,
            1,
        )  # Adjusted size
        ...
        self.att7 = AttentionGate3D(
            F_g=128, F_x=128, F_int=64
        )  # For dconv7 output and pt_features_16
        self.att8 = AttentionGate3D(
            F_g=64, F_x=64, F_int=32
        )  # For dconv8 output and pt_features_32
        self.att9 = AttentionGate3D(
            F_g=32, F_x=32, F_int=16
        )  # For dconv9 output and pt_features_64
        self.att10 = AttentionGate3D(
            F_g=1, F_x=1, F_int=1
        )  # For dconv10 output and pt_features_xyz_l
        ...
        self.fc13 = torch.nn.Linear(112, 16)
    ...
    def forward(self, data):
        ...
        if partial_cloud.shape[2] == 2:
            partial_cloud = torch.cat(
                [
                    partial_cloud,
                    torch.zeros(
                        partial_cloud.shape[0],
                        partial_cloud.shape[1],
                        1,
                        device=partial_cloud.device,
                    ),
                ],
                dim=2,
            )  # (B, N_partial, 2) -> (B, N_partial, 3)
        elif partial_cloud.shape[2] == 3:
            partial_cloud = torch.cat(
                [
                    partial_cloud[:, :, 0:2],
                    torch.zeros(
                ...
                dim=2,
            )  # (B, N_partial, 3) -> (B, N_partial, 3) replace z with 0
        ...
        n_partial_points = partial_cloud.size(1)

        dense_cloud_interp = (
            partial_cloud.unsqueeze(dim=2)
            .repeat(1, 1, 8, 1)
            .view(-1, self.n_dense_points, 3)
            .contiguous()
        )

        partial_cloud_features = (
            self.pointnet2_feature_extractor(
                partial_cloud.permute(0, 2, 1).contiguous()
            )
            .permute(0, 2, 1)
            .contiguous()
        )  # [B, 2048, 128]
        ...
        # # --- Bottleneck ---
        # if torch.isnan(pt_features_8).any() or torch.isinf(pt_features_8).any():
        #     logging.error(f"pt_features_8 has NaN: {torch.isnan(pt_features_8).any()}")
        #     logging.error(f"pt_features_8 has Inf: {torch.isinf(pt_features_8).any()}")
        #     raise ValueError("pt_features_8 has NaN or Inf")
        ...
        # print("partial_cloud max:", partial_cloud.max(), "partial_cloud min:", partial_cloud.min())
        ...
        point_offset = self.fc13(point_features).view(
            -1, self.n_dense_points, 2
        )  # Reshape (B, 2048, 16) -> (B, 16384, 2)
        # print(point_offset.size())    # torch.Size([B, 16384, 2])
...
if __name__ == "__main__":
    ...
    gridding_loss = GriddingLoss(  # lgtm [py/unused-local-variable]
        scales=[(256, 256, 1)], alphas=[1.0]
    )
    chamfer_dist = ChamferDistance()
    # gridding_scales = (128, 128, 1)
    ...
    dense_cloud_pred[:, :, 2] = 0
    
    if torch.isnan(dense_cloud_pred).any() or torch.isinf(dense_cloud_pred).any():
        logging.error(f"dense_cloud_pred has NaN: {torch.isnan(dense_cloud_pred).any()}")
        logging.error(f"dense_cloud_pred has Inf: {torch.isinf(dense_cloud_pred).any()}")
        raise ValueError("dense_cloud_pred has NaN or Inf")

    # dense_cloud_pred = torch.clamp(dense_cloud_pred, -0.9, 0.9)
    # gt_cloud = torch.clamp(gt_cloud, -0.9, 0.9)
    
    print("dense_cloud_pred max:", dense_cloud_pred.max())
    print("dense_cloud_pred min:", dense_cloud_pred.min())
    print("dense_cloud_pred mean:", dense_cloud_pred.mean())
    print("dense_cloud_pred std:", dense_cloud_pred.std())
    
    print("gt_cloud max:", gt_cloud.max())
    print("gt_cloud min:", gt_cloud.min())
    print("gt_cloud mean:", gt_cloud.mean())
    print("gt_cloud std:", gt_cloud.std())
    

    loss = (
        0.1 * gridding_loss(dense_cloud_pred, gt_cloud)
        + 0.8 * chamfer_dist(dense_cloud_pred, gt_cloud)
